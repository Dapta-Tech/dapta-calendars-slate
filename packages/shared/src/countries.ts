 ...
 * static input for the phone field's country selector (QA3 fix 2). Names are
 * NOT stored here: they come from Intl.DisplayNames at render time, which
 * localizes them for free. Flags are derived from the code (regional
 * indicator symbols), so each entry is just [iso2, dial].
 */
