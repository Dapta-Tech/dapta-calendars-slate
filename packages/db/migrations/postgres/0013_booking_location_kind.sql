...
--
-- ADD COLUMN (nullable, no default) is a fast metadata-only op in modern
-- Postgres — no long-lived lock on existing rows.

ALTER TABLE booking ADD COLUMN IF NOT EXISTS location_kind TEXT;
