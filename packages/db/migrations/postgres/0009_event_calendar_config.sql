...
-- ADD COLUMN (nullable, no default) and CREATE TABLE/INDEX are fast metadata-only
-- ops in modern Postgres — no long-lived lock on existing rows.

CREATE TABLE IF NOT EXISTS event_type_conflict_calendar (
  event_type_id         TEXT NOT NULL,
  connected_calendar_id TEXT NOT NULL,
  created_at            BIGINT NOT NULL,
  PRIMARY KEY (event_type_id, connected_calendar_id)
);
CREATE INDEX IF NOT EXISTS event_type_conflict_calendar_event_idx ON event_type_conflict_calendar (event_type_id);
CREATE INDEX IF NOT EXISTS event_type_conflict_calendar_calendar_idx ON event_type_conflict_calendar (connected_calendar_id);

ALTER TABLE event_type ADD COLUMN IF NOT EXISTS destination_calendar_id TEXT;
