 ...
 * second copy of `style.theme ?? <a literal>` at a call site is the whole class
 * of bug this file exists to prevent — and the literal moved once already.
 */

/**
 * What an ABSENT axis means, and it means the console.
 *
 * `brandingSchema.style` is `bookingPageStyleSchema.partial()`, so every config
 * saved before B2 carries no `theme` key at all. Those pages went dark -> light
 * when B2 shipped (#109) and this puts them straight back, which is the reason
 * ADR 0004's amendment lands days rather than months after it: almost nothing
 * has an explicitly saved `theme` yet, so this is an undo rather than a second
 * visible move. It is still a visible change to live pages, so it ships as one
 * change and is announced rather than discovered.
 *
 * The value is NOT declared here. It is re-exported from `@slate/shared`, which
 * the zod contract's own `.default()` is asserted equal to below, so the three
 * readers of this fact cannot drift apart.
 */
